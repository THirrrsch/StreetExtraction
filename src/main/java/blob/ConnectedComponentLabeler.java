...
class ConnectedComponentLabeler {
			...
			for (int j = roi.x; j < roi.x + roi.width; ++j) {
				...
				if (value == OBJECT) {
					...
					if (isNewInternalContour(j, i, proc)) {
						...
						try{
						Polygon innerContour = traceContour(j, i, proc, label, 2);
	...
	private Point nextPointOnContour(Point startPoint, ByteProcessor proc, int start) {
	...
	private void addWhiteBorder(ImagePlus img) {
		...
		for (int i = 0; i < oldproc.getHeight(); i++) {
			...
			int firstvalue = pixels[offset] & 255;
